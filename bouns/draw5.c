...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/25 02:54:21 by mel-ghar          #+#    #+#             */
/*   Updated: 2020/01/26 17:02:02 by mel-ghar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double			vert_raycast(t_dir dir, t_player *pl, double angel)
{
	...
	p.y = (!dir.sdown && !dir.sup) ? (pl->pos.x - p.x) *
	tan(angel * M_PI / 180) : 0;
				...
				t_point *inp, t_dir *dir)
{
	...
	delta.y *= ((dir->up && delta.y > 0) ||
	(dir->down && delta.y < 0)) ? -1 : 1;
	p->x += delta.x;
	p->y += delta.y;
	inp->y = pl->pos.y + p->y;
	inp->x = p->x;
